...
def updatenetwork():
    ...
    print("Keys", data.keys())
    #state, action, camera, reward, next_state, next_camera, terminated
    qradients = data['qradients']

    print("update q_network, length: ", len(qradients))
    for i in range(len(qradients)):
        qradients[i] = np.asarray(qradients[i])

    qradients = np.asarray(qradients)
    model.updateByGradient(qradients)
